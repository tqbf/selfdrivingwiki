...
| [`plans/json-render-source-providers.md`](plans/json-render-source-providers.md) | **json-render Source Providers.** Makes the wiki a [json-render](https://github.com/vercel-labs/json-render) render target: a vendored vanilla-JS form renderer (~200 lines, no React/npm) renders the form-primitives spec subset in a WKWebView. Phase 1 (shipped): `jsonrender-form.js` + `JSONRenderView` (WKWebView bridge) + `JSONRenderPayloadEncoder` (base64 spec injection) + `RedactionHelper` (secret scrubbing before log) + a dev-proof window. Phases 2–4 (planned): provider scripts + `manifest.json` discovery (`ProviderRegistry`), `ProviderScriptMaterializer` (runs scripts via bundled `bun`, output flows through `storeMaterialized`), connection tabs in the Sources sidebar (v38 `connections` table migration), keychain credentials + `wikictl` CLI commands. |
