...
1. **`tools/versiongen/main.swift`** → `Sources/WikiFSCore/GeneratedVersion.swift`
   — a checked-in Swift file with `appVersion`, `gitSHA`, `gitCommitCount`,
   `buildVersion`, `fullVersionString`. Regenerated by `make version` (a
   prerequisite of `build`/`check`/`test`/`release`). CI gate:
   `make check-version-gen`.
