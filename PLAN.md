...
| [`plans/coreml-minilm-design.md`](plans/coreml-minilm-design.md) | **Design (not yet implemented).** Replace `NLEmbedding` (512-dim) with a CoreML `all-MiniLM-L6-v2` (384-dim) on the Apple Neural Engine behind a new `Embedder` protocol — 100–1000× faster, safe off-main (no UI jank), no `std::bad_alloc` cliff. Includes the conversion/validation gate, an `embedding_meta`-driven dimension cutover wipe, and a phased plan (Phases 0–3). |
