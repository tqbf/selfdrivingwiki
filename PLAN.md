...
| [`docs/design-plans/2026-07-13-queue-engine.md`](docs/design-plans/2026-07-13-queue-engine.md) | **Queue Engine (in progress, Phase 1).** A persistent extraction/ingestion work queue (`queue.sqlite`) with durable state transitions, crash recovery, and pruning. Phase 1 (this doc) ships the `QueueStore` class and value types in `WikiFSCore` — no behavior change, no wiring. Phase 2 will add the `QueueEngine` actor + workers; later phases add the event stream, UI, and call-site integration. |
