...
| [`plans/resource-abstraction.md`](plans/resource-abstraction.md) | **#129 slice 2b (Phases A–C shipped; Phase D planned).** The Resource abstraction — a `Resource` leaf protocol + projection descriptors (`FlatResourceProjection`/`SingletonDoc`/`GeneratedIndex`/`NestedResourceProjection`) + a composed `changeToken` (per-kind fold contributors) — so a new resource kind gets File Provider projection + future MCP/REST for free, and the per-kind duplication in `Projection.swift` collapses. Decisions: genericize `Projection` not the `WikiStore` protocol (D2); one whole-DB token stays, construction becomes a contributor registry (D3); `Resource` covers the leaf, nesting is a descriptor (D1). Phases A–C shipped: **A** (token contributor registry), **B** (generic flat projection — pages+sources), **C** (singleton-doc + generated-index descriptors — root docs, manifest, jsonl). Phase D (bookmarks projection #125, nested-shape capstone) is next. Phase E (model reload-on-self-write, drops `origin`) deferred. See [`architecture-roadmap.md`](architecture-roadmap.md) §2.2/§5/§6. |
