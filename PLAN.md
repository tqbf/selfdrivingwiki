...
| [`plans/mermaid-diagrams.md`](plans/mermaid-diagrams.md) | Render ` ```mermaid ` fenced blocks as diagrams in the WKWebView readers (vendored mermaid v11, inline-only when a diagram is present) and teach the agent when to author one (`## Diagrams` section in `SystemPrompt.defaultBody`). |
