...
| [`plans/url-context-menu-add.md`](plans/url-context-menu-add.md) | **Implemented (`feat/url-context-menu-add`, PR #52).** Right-click an external http(s) link → **Add as Source** leads the context menu, opening the "Add from URL" sheet pre-filled (same Fetch + ingest as the toolbar button). New pure `.addAsSource` action (http(s) only; `mailto:` etc. excluded since `URLIngestService` rejects them); the sheet reaches the deep reader via a single `\.addURLHandler` environment value (no per-view wiring), and `AddFromURLSheet(store:initialURL:)` pre-fills the field. Native Textual reader only — `SourceWebView` (WKWebView) is a follow-up. |
