...
| [`plans/queue-engine.md`](plans/queue-engine.md) | **Queue Engine.** A persistent extraction/ingestion work queue (`queue.sqlite`) with durable state transitions, crash recovery, and pruning. The `QueueEngine` actor dispatches items to workers with per-provider concurrency limits and a per-wiki ingestion invariant. All PDF extraction and ingestion flows through the engine; a menu-bar status item shows queue state and provides pause/resume/halt/cancel/retry controls. The app drops to accessory (menu-bar-only) mode when the last window closes — queue work keeps running. |
