...
| [`plans/podcast-transcripts.md`](plans/podcast-transcripts.md) | **Apple Podcasts transcripts as sources.** Recognize a podcasts.apple.com episode URL in Add-from-URL and ingest its Apple-hosted TTML transcript as markdown. Isolated ObjC `podcast-token-helper` does the private-framework FairPlay signing; Swift `WikiFSCore` does the AMP + TTML + parse. Private API — dev/local only. Reference vendored at `docs/vendor/apple-podcast-transcript-downloader/`. |
