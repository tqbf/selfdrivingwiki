...
| [`plans/source-versioning-and-providers.md`](plans/source-versioning-and-providers.md) | **Design phase.** Foundational source model: source content versioning (append-only provider-fetch snapshots, byteless/external sources), a unified `SourceProvider` protocol absorbing Zotero/folder/URL/drag-drop + adding git/Tavily/Slack/archive, a derived/extraction **alternatives-family** (technique-tagged, comparable, active-selection), provenance grouping with inline media, and page→source `@version` pinning. Page/wiki versioning explicitly out of scope. |
