...
| [`plans/page-versions-and-workspaces.md`](plans/page-versions-and-workspaces.md) | **Multi-writer concurrency.** Eliminates the global agent edit lock by implementing snapshot-isolation semantics over a durable workspace substrate. 5-phase plan: W0 (page versions + CAS conflict detection) → W1 (workspaces + fast-forward merge) → W2 (real diff3 merge) → W3 (conflict resolution UI) → W4 (concurrency at scale). **W0 shipped** (v30): `page_versions` table (append-only, blob-backed bodies), `refs` rebuilt with CHECK on `kind` (drops the `owner_id` FK so `page-content` refs work), CAS save protocol (`appendPageVersion` + `PageConflictError`), `wikictl page history` / `revert`. See [`plans/w0-page-versions-cas.md`](plans/w0-page-versions-cas.md) for the implementation plan. |
