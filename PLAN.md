...
| [`plans/url-context-menu-add.md`](plans/url-context-menu-add.md) | **Implemented (`feat/url-context-menu-add`, PR #52).** Right-click an external http(s) link → **Add as Source** leads the context menu, opening the "Add from URL" sheet pre-filled (same Fetch + ingest as the toolbar button). New pure `.addAsSource` action (http(s) only; `mailto:` etc. excluded since `URLIngestService` rejects them); the sheet reaches the deep reader via a single `\.addURLHandler` environment value (no per-view wiring), and `AddFromURLSheet(store:initialURL:)` pre-fills the field. **Both readers:** the native Textual reader (`MarkdownPreview`) and the WKWebView large-source reader (`SourceWebView` — a `SourceDetailWebView` subclass overrides `willOpenMenu` since WebKit has no macOS context-menu API; DOM hit-test filters http(s)). **Issue #188 add-on:** an **Add Bookmark…** item (`.addBookmark` action) now follows Add as Source — fetches the link as a source via `WikiStoreModel.addURLForBookmark` (returns the source id; duplicates resolve to the existing source), then presents `BookmarkTargetPickerSheet` via a new `\.addBookmarkHandler` env value. |
