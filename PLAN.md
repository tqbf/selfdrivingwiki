...
| [`plans/address-bar-omnibox.md`](plans/address-bar-omnibox.md) | **Design.** Browser-style address bar above the tab strip: shows the active page's wikilink (`[[Title]]`) when idle, switches to debounced semantic search (`store.searchSimilar`) when focused. No agent query in v1 — pure location display + fast ranked search. Pure view-layer feature; no backend or schema changes. |
