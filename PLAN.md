...
| [`plans/persisted-chat-history.md`](plans/persisted-chat-history.md) | **Persisted chat history (issue #119, phase 1).** Ask/Edit conversations persist to the wiki's SQLite store (v23 `chats` + `chat_messages`, one row per typed `AgentEvent`), flushed at turn boundaries via a launcher transcript sink. `WikiSelection.chat(id)` opens a read-only transcript tab; the Agent sidebar lists recent conversations (delete via context menu); the Ask/Edit page gains PR #198's New Conversation button. `[[chat:…]]` links, `chats.jsonl`, the File Provider `chats/` tree, and multi-concurrent live sessions are documented follow-ups. |
| [`plans/conversation-ui.md`](plans/conversation-ui.md) | **Ask/Edit conversation UI (issue #119, phase 2).** Three pillars: (1) chat transcripts render source references (wikilinks, `#"quote"` anchors, `@vN` pins, `![[source:…]]` embeds, blob images) via a shared `WikiRenderContext`; (2) one `ConversationView` surface for live + persisted history with streaming; (3) continue a persisted conversation (`--resume` when possible, seeded-fresh fallback); (4) per-mode models/providers via named agent profiles. Phases A–D. |
| [`plans/agent-backend-port.md`](plans/agent-backend-port.md) | **Agent backend port (Phase 0).** Decouples the conversation layer from the Claude-CLI stream-json protocol: an `AgentBackend` port (`start`/`send`/`resume`/`cancel`) with `ClaudeCLIBackend` (actor) wrapping today's spawn/parse/encode behind a per-turn `AsyncStream<AgentEvent>`. Behavior-preserving; defers the ACP-vs-Polytoken-vs-CLI decision and makes a future backend swap transparent to the branch. Records the `.messageStop` turn-boundary contract, the `onExit` completion channel, and the D3 correction (`claude --resume` pins the model — model-switching is a backend capability). |
