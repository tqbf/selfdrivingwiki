...
| [`plans/graph-model-and-versioning.md`](plans/graph-model-and-versioning.md) | **Design of record for storage evolution.** Successor to the `source-versioning-and-providers.md` draft. Git's objects-vs-refs discipline in SQLite: content-addressed `blobs`, append-only `source_versions` + `provider_runs`, a mutable `refs` table for active-version pointers, role-typed reference edges (`cite`/`embed`/`render`, `![[…]]` embeds), extraction alternatives, ULID-canonical link targets (save-time normalization, display-at-render), `@vN` version pinning, and the Phase 0 concurrency substrate (method-atomic store, savepoint transactions, `WikiReadPool`). Records the "no CozoDB" decision. |
