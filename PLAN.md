...
| [`plans/connections.md`](plans/connections.md) | **Connections vs. provenance (design).** Names the four layers hidden in "source provider" (Kind / Connection / Item / Rendition) and locks the invariant: a **connection** (configured, credentialed origin instance — e.g. two Zotero accounts) is mutable config; **provenance** is immutable history that *snapshots* connection identity (stable ULID + label) onto the activity at ingest — no FK, so deleting a connection never touches provenance. Classifies the #261 origins: archives/local-git = connectionless, git-private = connection-optional, Tavily/Slack/Zotero = connection-required (Slack makes plural-per-kind the normal case). Item grouping key = `(connection_id, external_identity)`. Schema (nullable `connection_id`/`connection_label` on `activities` + a `connections` registry) deferred until the first connection-requiring origin — which *is* the Phase-3b credentials UX under its real name. Also specs the external query interface (`ConnectionQuerying` → `ItemDescriptor`/`RenditionDescriptor` → materializer factory; markdown-out is orchestration, never a materializer contract) and audits the 8 concrete code gaps (rendition ambiguity in Zotero `external_identity`, byteless unique-index collision under plural connections, refresh's missing credentials lookup, no core query seam, `isIngestable` vs format-dispatch drift, local-only Zotero resolution, missing provenance fields, `ensureAgent` first-writer-wins). Five-stage rollout with milestones: C1 provenance correctness (pre-schema, fixes the live Zotero rendition bug) → C2 query seam (`ConnectionQuerying` + `wikictl connection search`) → C3 the one migration (`connections` table, activity snapshot columns, byteless-index rebuild, Zotero as connection #1, the delete-survival invariant test) → C4 Tavily + the ingest orchestration ("query in, markdown out") → C5 connection-aware refresh + Zotero web-API path. |
| [`plans/chat-projection.md`](plans/chat-projection.md) | **Chat File Provider projection + `[[chat:…]]` wikilinks.** Chats (store v25) now project as a flat `chats/` tree (by-id + by-name), each rendered as a readable `.md` transcript via `ChatTranscriptRenderer`. Chat mutators route through `mutate()` and emit `.chat` events; a `ChatTokenContributor` (chat count + message count) extends the change token. `[[chat:Title]]` / `[[chat:<ULID>\|alias]]` wikilinks parse, render (`wiki://chat?title=…`), resolve (ghost vs live), navigate, and canonicalize — parallel to `[[source:…]]`. Includes `chats.jsonl` index, manifest count, `WIKI-STRUCTURE.md` chat entry. |
