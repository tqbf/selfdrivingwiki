...
public struct QueueEventEnvelope: Codable, Sendable {
    ...
    public static func chatEvent(chatID: ChatID, event: AgentEvent) -> QueueEventEnvelope {
    ...
    public static func chatState(
        chatID: ChatID, update: ChatStateUpdate
    ) -> QueueEventEnvelope {
    ...
    public static func chatAcpSessionId(
        chatID: ChatID, sessionId: AcpSessionID?
    ) -> QueueEventEnvelope {
        QueueEventEnvelope(
    ...
    public static func chatPendingPermission(
        chatID: ChatID, permission: PendingPermission?
    ) -> QueueEventEnvelope {
        let json: String? = permission.map { perm in
            let dict: [String: Any] = [
                "toolCallId": perm.toolCallId.rawValue,
