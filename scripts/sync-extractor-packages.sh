...
write_lock_file() {
  python3 - "$1" "$DEFUDDLE_SOURCE_DIGEST" "$PDF2MD_SOURCE_DIGEST" "$DOCX2MD_SOURCE_DIGEST" "$DOCLING_SOURCE_DIGEST" "$PODCAST_SOURCE_DIGEST" "$APPLE_PODCAST_SOURCE_DIGEST" "$DEFUDDLE_VERSION" "$DOCX2MD_MAMMOTH_VERSION" <<'PY'
import json, sys

path, defuddle_source, pdf2md_source, docx2md_source, docling_source, podcast_source, apple_podcast_source, defuddle_version, mammoth_version = sys.argv[1:10]
lock = {
    "comment": (
