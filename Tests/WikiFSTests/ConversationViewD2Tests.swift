...
struct ConversationViewD2Tests {
    ...
    @Test func retargetTab_preservesUUID_changesSelection() throws {
        ...
        #expect(model.tabs[0].selection == .ask)

        // Morph the tab in place: .ask → .chat(id)
        let chatID = page.id
    ...
    @Test func retargetTab_preservesTabOrder() throws {
        ...
        // Retarget the middle tab (.ask → .chat).
    ...
    @Test func retargetTab_unknownID_isNoOp() throws {
        ...
        model.retargetTab(id: UUID(), to: .edit)
    ...
    // MARK: - (c) Draft-state morph (Ask → .chat(id))
