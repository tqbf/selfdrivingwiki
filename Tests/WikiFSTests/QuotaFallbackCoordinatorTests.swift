...
struct QuotaFallbackCoordinatorTests {
    ...
    private func makeProvider(_ id: ProviderID, enabled: Bool = true) -> AgentProvider {
        AgentProvider(id: id, label: id.rawValue.capitalized, command: [id.rawValue], enabled: enabled)
    }
    ...
    func testMarkExhausted() {
        ...
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: reset, kind: .claudeSession)
        #expect(coord.isExhausted(ProviderID(rawValue: "claude-acp")))
    }

    @Test("isExhausted returns false for unknown provider")
    func testUnknownProviderNotExhausted() {
        let coord = QuotaFallbackCoordinator(quotaStateURL: makeQuotaURL())
        #expect(!coord.isExhausted(ProviderID(rawValue: "nonexistent")))
    }
    ...
    func testAutoRevival() {
        ...
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: pastDate, kind: .claudeSession)
    ...
    func testFirstLive() {
        let coord = QuotaFallbackCoordinator(quotaStateURL: makeQuotaURL())
        let chain = [
            makeProvider(ProviderID(rawValue: "claude-acp")),
            makeProvider(ProviderID(rawValue: "glm-acp")),
            makeProvider(ProviderID(rawValue: "gemini"))
        ]
        // No dead providers → first.
        #expect(coord.firstLive(in: chain)?.id == ProviderID(rawValue: "claude-acp"))

        // Mark claude dead → glm is first.
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: Date(timeIntervalSinceNow: 3600), kind: .claudeSession)
        #expect(coord.firstLive(in: chain)?.id == ProviderID(rawValue: "glm-acp"))

        // Mark glm dead too → gemini is first.
        coord.markExhausted(ProviderID(rawValue: "glm-acp"), resetTime: Date(timeIntervalSinceNow: 3600), kind: .zaiErrorCode(1310))
        #expect(coord.firstLive(in: chain)?.id == ProviderID(rawValue: "gemini"))
    }
    ...
    func testLongerWindowWins() {
        ...
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: longReset, kind: .claudeWeekly)
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: shortReset, kind: .claudeSession)
    ...
    func testNilResetTimeDefault() {
        let coord = QuotaFallbackCoordinator(quotaStateURL: makeQuotaURL())
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: nil, kind: .claudeSession)
    ...
    func testRecordPlanner() {
        ...
        coord.recordPlanner(providerId: ProviderID(rawValue: "glm-acp"), backend: backend)
        #expect(coord.plannerProviderId == ProviderID(rawValue: "glm-acp"))
    }
    ...
    func testFinishFallbackBackends() async {
        ...
        coord.recordBackend(primary, forProvider: ProviderID(rawValue: "claude-acp"))
        coord.recordBackend(fallback, forProvider: ProviderID(rawValue: "glm-acp"))
        await coord.finishFallbackBackends(excludingPrimaryProvider: ProviderID(rawValue: "claude-acp"))
        // Primary should be retained; fallback removed.
        #expect(coord.backends[ProviderID(rawValue: "claude-acp")] != nil)
        #expect(coord.backends[ProviderID(rawValue: "glm-acp")] == nil)
    ...
    func testFinishAllBackends() async {
        ...
        coord.recordBackend(backend1, forProvider: ProviderID(rawValue: "claude-acp"))
        coord.recordBackend(backend2, forProvider: ProviderID(rawValue: "glm-acp"))
