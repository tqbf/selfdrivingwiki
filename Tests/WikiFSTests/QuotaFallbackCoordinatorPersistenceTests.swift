...
struct QuotaFallbackCoordinatorPersistenceTests {
    ...
    func testDeadStateSurvivesRestart() {
        ...
        coord1.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: reset, kind: .claudeSession)
        #expect(coord1.isExhausted(ProviderID(rawValue: "claude-acp")))
        ...
        #expect(coord2.isExhausted(ProviderID(rawValue: "claude-acp")))
    }
    ...
    func testMultipleProvidersSurviveRestart() {
        ...
        let coord1 = QuotaFallbackCoordinator(quotaStateURL: url)
        coord1.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: reset1, kind: .claudeSession)
        coord1.markExhausted(ProviderID(rawValue: "glm-acp"), resetTime: reset2, kind: .zaiErrorCode(1310))
        #expect(coord1.isExhausted(ProviderID(rawValue: "claude-acp")))
        #expect(coord1.isExhausted(ProviderID(rawValue: "glm-acp")))

        // Simulate app restart.
        let coord2 = QuotaFallbackCoordinator(quotaStateURL: url)
        #expect(coord2.isExhausted(ProviderID(rawValue: "claude-acp")))
        #expect(coord2.isExhausted(ProviderID(rawValue: "glm-acp")))
    }
    ...
    func testRevivedProviderStaysRevived() {
        ...
        coord1.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: pastDate, kind: .claudeSession)
    ...
    func testAliveAfterExpiry() {
        ...
        let coord = QuotaFallbackCoordinator(quotaStateURL: url)
        coord.markExhausted(ProviderID(rawValue: "claude-acp"), resetTime: pastExpiry, kind: .claudeSession)
    ...
    func testKindSurvivesRoundTrip() {
        ...
        let coord1 = QuotaFallbackCoordinator(quotaStateURL: url)
        coord1.markExhausted(ProviderID(rawValue: "glm-acp"), resetTime: Date(timeIntervalSinceNow: 3600), kind: .zaiErrorCode(1310))
    ...
    func testMissingFileStartsEmpty() {
        ...
        let coord = QuotaFallbackCoordinator(quotaStateURL: url)
        #expect(!coord.isExhausted(ProviderID(rawValue: "claude-acp")))
        #expect(!coord.isExhausted(ProviderID(rawValue: "glm-acp")))
    }
}
#endif // os(macOS)
