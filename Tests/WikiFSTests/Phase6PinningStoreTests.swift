...
struct Phase6PinningStoreTests {
    ...
    @Test func replaceLinksUnpinnedWritesNULL() throws {
        ...
        try store.appendProcessedMarkdown(sourceID: source.id, content: "v1", origin: "t", note: nil)
    ...
    @Test func ordinalResolvesChronologically() throws {
        ...
        let v1 = try store.appendProcessedMarkdown(sourceID: source.id, content: "oldest", origin: "t", note: nil)
        _ = try store.appendProcessedMarkdown(sourceID: source.id, content: "mid", origin: "t", note: nil)
        let v3 = try store.appendProcessedMarkdown(sourceID: source.id, content: "newest", origin: "t", note: nil)
    ...
    @Test func ordinalStableUnderAppend() throws {
        ...
        _ = try store.appendProcessedMarkdown(sourceID: source.id, content: "v4", origin: "t", note: nil)
        let chain = try store.sourceDerivedChains()[source.id]!
    ...
    @Test func processedMarkdownVersionReturnsCorrectRow() throws {
        ...
        _ = try store.appendProcessedMarkdown(sourceID: source.id, content: "first", origin: "t", note: nil)
        let v2 = try store.appendProcessedMarkdown(sourceID: source.id, content: "second", origin: "t", note: nil)
