...
struct RendererPackageDocumentationTests {
    ...
    func maintainerSkillAndGuideDescribeCurrentAvailabilityPolicy() throws {
        ...
        #expect(guide.contains("It permits WebAssembly compilation through `'wasm-unsafe-eval'`, without JavaScript `eval`."))
        #expect(guide.contains("It permits a package to fetch its own declared, hash-pinned assets through the `renderer-package` scheme."))
        #expect(guide.contains("It blocks network origins, frames, workers, objects, forms, and base URLs."))
        #expect(guide.contains("Run WebAssembly on the main thread."))
        #expect(skill.contains("Run WebAssembly on the main thread; the CSP keeps `worker-src` closed."))
    ...
    func rendererDocumentationRecordsEmbeddingRoles() throws {
        ...
        #expect(guide.contains("The reader keeps a DOM fallback for renderer-backed images."))
        #expect(guide.contains("Renderer packages add read-only views for page content."))
