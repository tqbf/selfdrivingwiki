...
struct PageVersionTests {
    ...
    /// `user_version`. The v49 step adds GRDB-owned tracked repository
    /// metadata after the v46/v47/v48 chat and provenance migrations.
    @Test func v44SchemaVersionAfterMigration() throws {
        #expect(GRDBWikiStore.schemaVersion == 49,
                "schemaVersion must report 49 after tracked repository migration")
        let store = try tempStore()
