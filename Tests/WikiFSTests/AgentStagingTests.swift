...
struct AgentStagingTests {
  ...
  @Test func shellSafeLeafPreservesSimpleAsciiStem() {
    #expect(AgentStaging.shellSafeLeaf(name: "Neuralwatt Cloud Platform", sourceID: SourceID(rawValue: "01KXYMP7J6HZ3E34ZZX02HKS1F"), ext: "html")
            == "Neuralwatt-Cloud-Platform--01KXYMP7J6HZ3E34ZZX02HKS1F.html")
    #expect(AgentStaging.shellSafeLeaf(name: "Trip Report", sourceID: SourceID(rawValue: "01ABCDEF"), ext: "pdf")
            == "Trip-Report--01ABCDEF.pdf")
    #expect(AgentStaging.shellSafeLeaf(name: "Home", sourceID: SourceID(rawValue: "01KV6EAH"), ext: "md")
            == "Home--01KV6EAH.md")
  }
  ...
  @Test func shellSafeLeafReplacesSpacesAndShellMetacharactersWithDashes() {
    ...
    #expect(AgentStaging.shellSafeLeaf(name: "Cost & Revenue (Q3)", sourceID: SourceID(rawValue: "01K"), ext: "md")
            == "Cost-Revenue-Q3--01K.md")
    #expect(AgentStaging.shellSafeLeaf(name: "name with $vars", sourceID: SourceID(rawValue: "01K"), ext: "pdf")
            == "name-with-vars--01K.pdf")
    // The shell metacharacters " ` | ; are all replaced with '-'.
    #expect(AgentStaging.shellSafeLeaf(name: "a\"b`c|d;e", sourceID: SourceID(rawValue: "01K"), ext: "txt")
            == "a-b-c-d-e--01K.txt")
  }

  @Test func shellSafeLeafReplacesPathSeparatorsWithDashes() {
    // `escapeTitle` already replaces '/' and ':' with '-' (FilenameEscaping.swift).
    #expect(AgentStaging.shellSafeLeaf(name: "a/b:c", sourceID: SourceID(rawValue: "01K"), ext: "txt")
            == "a-b-c--01K.txt")
    ...
    #expect(AgentStaging.shellSafeLeaf(name: ".hidden", sourceID: SourceID(rawValue: "01K"), ext: "md")
            == "_.hidden--01K.md")
  }
  ...
  @Test func shellSafeLeafFallsBackToUntitledForEmptyOrAllMetacharName() {
    ...
    #expect(AgentStaging.shellSafeLeaf(name: "", sourceID: SourceID(rawValue: "01K"), ext: "md")
            == "untitled--01K.md")
    #expect(AgentStaging.shellSafeLeaf(name: "  ", sourceID: SourceID(rawValue: "01K"), ext: "md")
            == "untitled--01K.md")
    #expect(AgentStaging.shellSafeLeaf(name: "$$$", sourceID: SourceID(rawValue: "01K"), ext: "md")
  ...
  @Test func shellSafeLeafSanitizesExtension() throws {
    ...
    #expect(AgentStaging.shellSafeLeaf(name: "Doc", sourceID: SourceID(rawValue: "01K"), ext: "MD")
            == "Doc--01K.md")  // lowercased
    #expect(AgentStaging.shellSafeLeaf(name: "Doc", sourceID: SourceID(rawValue: "01K"), ext: ".md")
            == "Doc--01K..md")  // '.' kept (allowed); still shell-safe
    #expect(AgentStaging.shellSafeLeaf(name: "Doc", sourceID: SourceID(rawValue: "01K"), ext: "pdf ")
    ...
    #expect(AgentStaging.shellSafeLeaf(name: "Doc", sourceID: SourceID(rawValue: "01K"), ext: "md;rm")
            == "Doc--01K.mdrm")
    let shellSafePattern = try NSRegularExpression(pattern: "^[A-Za-z0-9._-]+$")
    for ext in ["MD", ".md", "pdf ", "md;rm", "md`rm", "md$rm"] {
      let leaf = AgentStaging.shellSafeLeaf(name: "Doc", sourceID: SourceID(rawValue: "01K"), ext: ext)
  ...
  @Test func shellSafeLeafOmitsExtensionWhenEmpty() {
    ...
    #expect(AgentStaging.shellSafeLeaf(name: "Notes", sourceID: SourceID(rawValue: "01K"), ext: "")
            == "Notes--01K")
  }

  @Test func sourceIDFilenameContractIsUnchanged() {
    ...
    let leaf1 = AgentStaging.shellSafeLeaf(name: "Same Name", sourceID: SourceID(rawValue: "01KXYMP7J6HZ3E34ZZX02HKS1F"), ext: "md")
    let leaf2 = AgentStaging.shellSafeLeaf(name: "Same Name", sourceID: SourceID(rawValue: "01KXYMP7J6HZ3E34ZZX02HKS1G"), ext: "md")
    #expect(leaf1 != leaf2)
  ...
  @Test func stagesMultipleSourcesWithDescriptiveShellSafeLeaves() throws {
    ...
    let sources: [(bytes: Data, ext: String, name: String, sourceID: SourceID)] = [
      (Data("first".utf8),  "md",   "Neuralwatt Cloud Platform", SourceID(rawValue: "01KXYMP7J6HZ3E34ZZX02HKS1F")),
      (Data("second".utf8), "pdf",  "MCR Protocol",               SourceID(rawValue: "01KXYMKF1EF820Y5KZ6FTBAQRS")),
    ]
  ...
  @Test func stagedSourceLeavesAreShellSafe() throws {
    ...
    let sources = cases.map { (Data("x".utf8), $0.ext, $0.name, SourceID(rawValue: ulid)) }
    let paths = try AgentStaging.stageSources(sources, in: scratch)
  ...
  @Test func stateFileRendersTitlesIndexAndLog() {
    ...
    #expect(md.contains("Self Driving Wiki includes and manages the read-only Excalidraw renderer automatically."))
    #expect(md.contains("Every compatible validated installed renderer is available to every wiki."))
    #expect(md.contains("directories only"))
