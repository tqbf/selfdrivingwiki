...
/// Tests that `WikiStoreModel.ingestURL` ROUTES an Apple Podcasts episode link to
...
struct PodcastIngestRoutingTests {
    ...
    struct ExplodingFetcher: URLIngestService.URLResourceFetcher {
        func fetch(_ url: URL) async throws -> URLIngestService.FetchResponse {
            ...
            throw URLIngestService.IngestError.empty
        }
    }
    ...
    @Test func episodeURLRoutesToTranscriptPipelineAndStoresMarkdown() async throws {
        ...
        let outcome = try await model.ingestURL(
    ...
    @Test func missingHelperGivesAClearError() async throws {
        ...
        await #expect(throws: URLIngestService.IngestError.self) {
            try await model.ingestURL(
    ...
    @Test func nonPodcastURLStillUsesHTMLFetcher() async throws {
        ...
        // is never consulted).
        struct HTMLFetcher: URLIngestService.URLResourceFetcher {
            func fetch(_ url: URL) async throws -> URLIngestService.FetchResponse {
