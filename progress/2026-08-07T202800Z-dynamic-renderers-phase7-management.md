...
Removal retains a redacted `.removed` machine record and deletes the package payload. It does not delete source data or wiki renderer preferences. Safe-mode reset and registry refresh use the existing host. A source version choice writes an exact typed renderer preference, so it does not replace an active pane pin.
