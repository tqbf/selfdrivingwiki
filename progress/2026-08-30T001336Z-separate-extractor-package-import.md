...
accessibility identifiers remain unchanged. The visible import title is now the
disclosure label, and its full row is clickable. Active and failed package rows
also use full-width disclosure labels. Read-only and headless views still hide
the import section when the app does not provide an import action.
...
- The focused settings suite passed again after the row interaction change.
