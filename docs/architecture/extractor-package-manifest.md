...
The host resolves the command name against a search list before spawn and launches the absolute path: the user's login-shell `PATH` directories first (resolved once per process by spawning the login shell, since a GUI process does not inherit it), then standalone install locations — `~/.bun/bin`, `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, `/bin` — and finally `~/.local/share/mise/shims`. A mise shim is a symlink to the `mise` selector binary and dispatches on `argv[0]`, so it is launched by its own path; the host sets `MISE_DATA_DIR`, `MISE_CONFIG_DIR`, and the tool's version pin (`MISE_<TOOL>_VERSION=latest`) because the process runs with a sandboxed `HOME`. Bun and uv are the runtimes the reviewed packages use. They are optional user-installed tools, not app dependencies. An absent runtime produces a typed `missing-runtime` failure, and fallback stays available.
