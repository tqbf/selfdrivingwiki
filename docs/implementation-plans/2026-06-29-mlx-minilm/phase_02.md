...
The model is **not committed** — `build.sh` ensures it's present by running the
...
> skip the block (the app then falls back to NLEmbedder — see Phase 2 selection).

```bash
git add build.sh
git commit -m "feat: ensure MLX model present + bundle it in build.sh (downloaded, gitignored)"
```
