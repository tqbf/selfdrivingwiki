...
# ingest). Optional — a toolchain that can't build the ObjC target just omits it,
# and podcast ingest surfaces a clear "not available" message.
...
PLIST
  ...
  # podcast-token-helper is a nested Mach-O — sign it or the outer app's seal
  # fails. No entitlements: it dlopens Apple-signed system frameworks (platform
  # binaries, allowed under library validation) in its own process.
  if [ -x "${HELPERS_DIR}/${PODCAST_HELPER_NAME}" ]; then
    codesign --force --timestamp=none --sign "${IDENTITY}" \
