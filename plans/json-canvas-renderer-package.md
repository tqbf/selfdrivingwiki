...
The viewer is read-only, package-local, and accessible. It supports text, file, link, and group nodes; node and edge colors; edge labels with arrowheads; deterministic order and z-order; pan, zoom, fit, selection, and keyboard traversal; light and dark appearance; and Reduce Motion. Version 1.1.6 repairs the rendering model: correct side-aware Bézier edge geometry, full JSON Canvas 1.0 field parsing with defaults, bounded Markdown text layout, image file nodes and group background images through the host-pinned `asset.read` allowlist, initial fit-to-window, WebKit-safe colored edge paths and markers, and nearest-facing automatic diagonal edge anchors.
...
- Version 1.0.1 (manifest revision 4) and 1.1.0 are preserved in repository history with their package hashes as stability contracts; their rendering claims are superseded by 1.1.1.
