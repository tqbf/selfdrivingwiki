...
- **Provenance-sibling resolution** (draft §media, kept): a website snapshot's
  ...
  versions *in the same provider run* by `original_path`. *(Phase 4 amendment —
  Decision D4: absolute image srcs are normalized to their relative
  `original_path` at materialize time so the snapshot is self-contained and
  resolvable offline. The immutable content-addressed blob — not the src string
  — is the reproducibility anchor.)* `original_path` collisions within a run
  (draft open question #5, resolved): first match in ULID order wins, and the
  fetch layer disambiguates at materialize time by suffixing
  (`images/foo-2.png`) — the same rule `MarkdownFolderReader` already applies
  to duplicate filenames. *(Phase 4 shipped: `WebsiteSnapshotExtractor` +
  `addSnapshotImage` per-snapshot image sources + `siblingImageResolvers` +
  `imageResolver` on `MarkdownHTMLRenderer`. Snapshot-aware refresh deferred.)*
...
5. **v22: `source_links` rebuild** as the rowid table + `COALESCE`'d unique
   ...
   > **Status (Phase 4 foundation, 2026-07-06):** shipped. `migrateV21ToV22()`
   > adds `sources.role` (default `'primary'`) and rebuilds `source_links` to
   > the §4.4 rowid + role/pin shape + `source_links_edge` unique index. The
   > `SourceRole` enum + `SourceSummary.role`/`isPrimary` seam widen the value
   > type and read/write paths; media sources are filtered from the main
   > Sources list via `SourcesContainerView.visibleSources`.
   >
   > **Status (Phase 4a embeds, 2026-07-06):** shipped. `![[source:…]]` embed
   > parsing + binary content rendering landed: `WikiLinkParser.ParsedLink.isEmbed`
   > + `WikiLinkSpan.isEmbedPrefix` detect the `!` prefix; `replaceLinks` writes
   > `role='embed'` edges; `WikiLinkMarkdown.linkified()` emits inline HTML
...
| ↳ *Phase 4 foundation shipped (v22, 2026-07-06).* `sources.role` + `source_links` §4.4 rebuild + `source_links_edge` index landed; `SourceRole`/`isPrimary` seam + media filtering in the Sources UI. *Phase 4a embeds (2026-07-06):* `![[source:…]]` parsing + `role='embed'` edges + `BlobSchemeHandler` binary serving (`wiki-blob://`) shipped. *Phase 4b byteless external embeds (2026-07-06):* provider-player iframes (YouTube/Vimeo/Spotify/SoundCloud), direct-remote native `<audio>`/`<video>` (mp3/mp4/HLS), and the Apple Podcasts embed player — all via one shared byteless mechanism (`SourceEmbedDescriptor` + `embedDescriptors()` query + `ExternalEmbed` dispatch table + target-first `embedHTML` ordering). No schema change (`external_identity`/`mime_type`/`thumbnail_hash` already existed); byteless sources, `role: .primary`. *Phase 4 close-out — website snapshot `original_path` resolution (2026-07-06):* `WebsiteSnapshotExtractor` downloads page images off-main (caps: 20 MB/image, 30 images, 50 MB total), rewrites `<img src>` to relative `original_path` at the token level (D4 amends §7), `addSnapshotImage` stores per-snapshot `.media` sources (blob-only dedup), `siblingImageResolvers()` + `imageResolver` on `MarkdownHTMLRenderer.render` resolve relative srcs to `wiki-blob://source/<id>` at render. No schema change (`original_path` shipped v20). Snapshot-aware refresh (re-snapshotting images) guarded — `RefreshError.snapshotWithImages`. Remaining deferred: `json-render` generative-UI spec, `apple-ttml` transcript-level PROV, snapshot-aware refresh. | | |
