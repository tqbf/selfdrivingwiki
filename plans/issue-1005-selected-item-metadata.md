...
Chat metadata shows created, updated, message count, latest turn provider, model, started, finished, duration, status, input, output, total, thought, cache read, cache write, and cost when available. `ChatDetailView` reads one durable `ChatUsageSummary` through its hydration path: its `latestTurn` feeds the explicitly titled **Latest turn** section, while aggregate counters and matching-currency cost render separately under **Persisted conversation totals**. The daemon overlay replaces only that latest turn by `ChatTurnID`; it never changes or re-sums durable conversation totals. Legacy chats omit unavailable rows. `ChatSummary.createdAt` is only the conversation creation date, never a later turn start fallback.
...
| AC-06 | `Tests/WikiFSAppTests/MetadataPanelHostedTests.swift`: `hostedInspectorAt180UsesStackedRows()`, `hostedInspectorAt500UsesGridRows()`, `hostedThresholdUsesStackedAt299AndGridAt300()`, `hostedInspectorSupportsWrappedValues()`, `hostedInspectorExposesPickerLabel()`, `hostedRowsExposeLabelsValuesAndHints()`, `hostedActionsAreKeyboardAndVoiceOverReachable()`, `hostedIdentifiersAreSelectable()` |
...
| AC-26 | `Tests/WikiFSAppTests/MetadataCrossProcessRefreshTests.swift`: `eventBusChangeAdvancesHydrationKeyAndRehydratesSelectedChat()`, `daemonSyncRefreshesLiveChatWithoutStoreRead()` |
